...
| **단련 · the proving ground** (`#/ko/quiz`) | The folios, run backwards into questions. The thesis: every instrument in the atlas is a **selector → answer** machine — pick a verb + a tense, the forge hands you 갔어요; point at a meaning, the grid hands you 이것; pick a noun + a role, the gate hands you 책은. The proving ground shows the empty slot and asks you to *choose what fills it*, in the script drill's exact gamified loop (challenge → answer → feedback). **The rack** — a swappable menu of decks, one salient at a time (the others recede); each card carries its own best-streak + learned count. **The cycle** — four-option recall, questions weighted toward the cards you know least, and the three distractors drawn FIRST from the target's *near* cohort (other tenses of the same verb, other pointers in the same row) so a right answer means you chose the **axis**, not just recognized the word; a streak glows, clearing a scope or a streak of ten lights a lantern, mastery lamps fill faint→gold and persist (`atlas.ko.quiz.v1`, backend-shaped). Three decks ship: **동사** verb conjugation (the forge inverted — 가다 + past → 갔어요), **이·그·저** the pointer grid (the deixis row inverted — *that way · yonder* → 저쪽, the ano·kono·sono·dono drill exactly), **조사** the 받침 gate (a noun + a role → the right particle with the 받침-correct shape baked in). The **이·그·저** deck carries a second face — a *flashcards | the grid* toggle hands its native **4×6 machine** (prefix rows × category columns) over, run three ways: **choose** (a cell lights → pick its word), **type** (a cell lights → produce its word on the keyboardless IME — romaja→한글, graded against the composed word since the grid's readings are pronunciation-RR), and **locate** (a word is named → click its square). The cells stay blank; the row prefixes and column suffixes are the coordinates you read — and an **axis script** toggle hides those prefixes/suffixes (이/그/저 · 것/기/…) for a real quiz, leaving only the English role/label. Decks are **data, not components** (`src/data/quizzes/`): each is assembled from the SAME hand-checked module its live instrument reads (`koreanData.js`, `koreanDeixis.js`), so a card can never disagree with the forge — surface forms are re-pointed, never re-typed. The deeper *assemble* mode (dials that pool — verb + register + tense) drops into this same rack next. |
...
src/
  ...
  components/
    ...
    quiz/                  the proving ground (단련 / 鍛錬 — folios run backwards into questions)
      ...
      GridQuiz.jsx         the grid cycle — the 4×6 deixis machine run choose / type / locate
  ...
  data/
    ...
    quizzes/               the proving ground's decks — folios reshaped into questions
      ...
      koreanQuizzes.js     KO_DECKS — 동사 (verbs) · 이·그·저 (deixis) · 조사 (the 받침 gate)
