...
Hash-routed: **language → folio (page)**. e.g. `#/ko/hangul`, `#/ko/phonetics`, `#/ko/grammar`, `#/ko/deixis`, `#/ko/verbs`, `#/ko/forms`, `#/ko/particles`, `#/ko/quiz`, `#/ja/kana`, `#/ja/phonetics`, `#/ja/grammar`, `#/ja/deixis`, `#/ja/verbs`, `#/ja/forms`, `#/ja/particles`, `#/ja/quiz`.
...
| **단련 · the proving ground** (`#/ko/quiz`) | The folios, run backwards into questions. The thesis: every instrument in the atlas is a **selector → answer** machine — pick a verb + a tense, the forge hands you 갔어요; point at a meaning, the grid hands you 이것; pick a noun + a role, the gate hands you 책은. The proving ground shows the empty slot and asks you to *choose what fills it*, in the script drill's exact gamified loop (challenge → answer → feedback). **The rack** — a swappable menu of decks, one salient at a time (the others recede); each card carries its own best-streak + learned count. **The cycle** — four-option recall, questions weighted toward the cards you know least, and the three distractors drawn FIRST from the target's *near* cohort (other tenses of the same verb, other pointers in the same row) so a right answer means you chose the **axis**, not just recognized the word; a streak glows, clearing a scope or a streak of ten lights a lantern, mastery lamps fill faint→gold and persist (`atlas.ko.quiz.v1`, backend-shaped). Six decks ship: **동사** verb conjugation (the forge inverted — 가다 + past → 갔어요), **활용표** the conjugation table (the forge × the register dial — one verb's whole **register × tense** grid, a *quiz within verb*), **이·그·저** the pointer grid (the deixis row inverted — *that way · yonder* → 저쪽, the ano·kono·sono·dono drill exactly), **조사** the 받침 gate (a noun + a role → the right particle with the 받침-correct shape baked in), **서랍** the particle drawers (the cabinet inverted — a *job* + its Japanese twin → the right particle: *than* (より) → 보다, the JP cue riding the bridge so it hides for self-testing), and **빈칸** fill the blank (the cabinet's *specimen* run backwards — the particle lifted out of its own sentence, 고양이가 집▢ 있어요 → 에, the Japanese twin riding the bridge as the disambiguator when a slot could take more than one; the particles the cabinet gives several senses — 에/에서/(으)로/(이)나 — pull in its example *faces* too, so they're drilled across uses, not one frozen line, for 45 cards in all). The three particle decks split the folio's axes — 조사 drills which *shape*, 서랍 which *word* in the abstract, 빈칸 which *word* in the wild — and all three reuse the cabinet's **five drawers** (뼈대·자리·짝·초점·사람) as their scope chips, the *near*-miss distractors drawn from the same drawer (the three "and"s 와·하고·랑; the focus set's 도·만·밖에·조차·마저). The **이·그·저** deck carries a second face — a *flashcards | the grid* toggle hands its native **4×6 machine** (prefix rows × category columns) over, run three ways: **choose** (a cell lights → pick its word), **type** (a cell lights → produce its word on the keyboardless IME — romaja→한글, graded against the composed word since the grid's readings are pronunciation-RR), and **locate** (a word is named → click its square). The cells stay blank; the row prefixes and column suffixes are the coordinates you read — and an **axis script** toggle hides those prefixes/suffixes (이/그/저 · 것/기/…) for a real quiz, leaving only the English role/label. The **활용표** deck wears the same grid machine, with a third dial — the verb — handled as a **selector strip** (가다 · 보다 · 먹다 · 하다 · 듣다, one representative per pattern) over a **register (합쇼체·해요체·해체·해라체) × tense (present·past·future)** board, run **choose** and **locate** (type is omitted — conjugated forms carry 받침/liaison spelling the romaja IME can't fairly grade); the column hints are the shared tense markers (∅ · 았/었 · ㄹ 거), so it realizes the *assemble* mode the coda named (verb + register + tense). Decks are **data, not components** (`src/data/quizzes/`): each is assembled from the SAME hand-checked module its live instrument reads (`koreanData.js`'s `CONJ_VERBS` table, `koreanDeixis.js`, `koreanParticles.js`'s `PARTICLE_FAMILIES`), so a card can never disagree with the forge, the gate, or the cabinet — surface forms are re-pointed, never re-typed. The Japanese mirror (plain\|polite × tense, derived from `japaneseVerbs.js`) drops into the 鍛錬 rack next. |
...
| **鍛錬 · the proving ground** (`#/ja/quiz`) | The Korean proving ground's mirror — the Japanese folios run backwards into questions, on the same language-blind apparatus (`components/quiz/`). The rack swaps between decks (one salient), the kana drill's gamified cycle runs below, and the **한국어 twin rides the bridge toggle as a cue**. Three decks ship: **動詞活用** verb conjugation (the forge inverted — 飲む + past → 飲んだ, distractors the SAME verb's other four forms, scoped by class 一段/五段/不規則), **こそあど** the pointer grid (*that way* → あちら, the column's other pointers as near misses; the same *flashcards | the grid* toggle with **choose / type / locate** modes as Korean, the free-entry engine being kana), **形容詞** the い/な bench (高い + past → 高かった, scoped い vs な). Same engine, store (`atlas.ja.quiz.v1`), and data-not-components rule as Korean; decks are assembled from `japaneseVerbs.js` / `japaneseDeixis.js` / `japaneseAdjectives.js`. |
...
src/
  ...
  pages/
    ...
    KoreanQuizzes.jsx      ko folio — the proving ground (rack + the challenge cycle)
    ...
    JapaneseQuizzes.jsx    ja folio — the proving ground (mirror of #/ko/quiz)
  components/
    ...
    quiz/                  the proving ground (단련 / 鍛錬 — folios run backwards into questions)
      ProvingGround.jsx    the language-blind apparatus: owns the store, holds one deck salient
      DeckRack.jsx         the swappable menu of decks (one active, each a mini-scoreboard)
      ActiveDeck.jsx       the runner: format toggle (flashcards | the grid) for grid-backed decks
      QuizStage.jsx        the flashcard cycle — challenge → answer → feedback (the script drill, generalized)
      GridQuiz.jsx         the grid cycle — a 2-axis board run choose / type / locate;
                           the deixis 4×6, or the verb 활용표 (an optional `pick`
                           selector redraws register × tense per chosen verb)
      quizEngine.js        shared question-maker (weighted target + near-miss distractors)
      useQuizStore.js      per-card mastery + per-deck best streak — localStorage atlas.<lang>.quiz.v1
  ...
  data/
    ...
    koreanData.js          ko content (hangul + RR + JP bridges, hand-checked) —
                           loom, gate, forge, register dial, 안/못, and the
                           CONJ_VERBS conjugation table (register × tense, the
                           forge × the dial crossed; feeds the 활용표 quiz deck)
    ...
    quizzes/               the proving ground's decks — folios reshaped into questions
      schema.js            the QuizDeck/QuizCard/QuizFace contract + small build helpers
      koreanQuizzes.js     KO_DECKS — 동사 (verbs) · 활용표 (the conjugation
                           table: register × tense, verb picker) · 이·그·저
                           (deixis) · 조사 (the 받침 gate) · 서랍 (the particle
                           drawers: job + JP twin → particle, scoped by drawer)
                           · 빈칸 (fill the blank: the particle cloze-removed
                           from its own cabinet specimen, same drawer scopes)
      japaneseQuizzes.js   JA_DECKS — 動詞 (verbs) · こそあど (deixis) · 形容詞 (adjectives)
  styles/
    ...
    quiz.css               the proving ground (단련 / 鍛錬) — the deck rack + the challenge cycle
tools/
