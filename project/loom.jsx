/* loom.jsx — the Loom.
   Drag the [phrase·particle] tiles into any order; the verb stays anchored
   at the end; the meaning is read from the PARTICLES, not the order.
   Click a swappable particle (が ⇅ を) to re-cast who does what.

   LoomInstrument owns the specimen switch; LoomBoard (keyed by specimen id)
   owns the play state, so switching specimens remounts with fresh state.
*/

function LoomInstrument({ showReadings }) {
  const specimens = window.LOOM_SPECIMENS;
  const [specimenId, setSpecimenId] = React.useState(specimens[0].id);
...
function LoomBoard({ specimen, showReadings }) {
  const [order, setOrder] = React.useState(specimen.tiles.map(t => t.id));
  const [swaps, setSwaps] = React.useState({});           // tileId -> swap index
  const [eureka, setEureka] = React.useState(null);
  const [shake, setShake] = React.useState(false);

  const [drag, setDrag] = React.useState(null);           // { id, dx, targetIndex }
  const dragRef = React.useRef(null);                     // mutable cache during a drag
  const tileEls = React.useRef({});
  const reorderedOnce = React.useRef(false);
  const swappedOnce = React.useRef(false);
  const anchoredOnce = React.useRef(false);
  ...
  const transformFor = (id, idx) => {
    ...
    const r = idx < d.index ? idx : idx - 1;            // index among remaining
  ...
  const cycleParticle = (id) => {
    ...
    if (specimen.coupledSwap) {
      // re-cast every swappable tile together → always exactly one が, one を
  ...
  // predicate "tries to move" → it won't. head-final lesson.
  const nudgeAnchor = () => {
    setShake(true);
  ...
  return (
    <>
      ...
      <div className="loom-line">
        ...
        >
          {order.map((id, idx) => {
            const t = resolve(id);
            if (!t) return null;
            const role = window.GRAM_ROLES[t.role];
      ...
      <div className="role-legend">
        {rolesPresent.map(r => {
          const role = window.GRAM_ROLES[r];
          return (
      ...
      <div className="loom-controls">
        ...
        <button className="text-btn" onClick={() => { setOrder(specimen.tiles.map(t => t.id)); setSwaps({}); }}>
          ↺ reset arrangement
        </button>
      </div>
    </>
...
window.LoomInstrument = LoomInstrument;
