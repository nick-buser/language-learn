/* verbdial.jsx — the Verb Dial.
   One verb, one subject (私). Turn the dial through plain → passive →
   causative → causative-passive and watch 私's ROLE flip while the
   grammatical subject never moves. The actor diagram re-aims itself.
*/

function VerbDial() {
  const dial = window.VERB_DIAL;
  const [i, setI] = React.useState(0);
  const state = dial.states[i];

  // arrow geometry by direction
  ...
  return (
    <div className="dial-stage" data-screen-label="Verb dial">
      <div className="loom-prompt" style={{ marginTop: 0 }}>
        The verb <b style={{ color: "var(--ink)", fontFamily: "var(--font-cjk-serif)", fontWeight: 500 }}>{dial.verb}</b>{" "}
        <span style={{ fontStyle: "normal", color: "var(--ink-faded)", fontFamily: "var(--font-mono)", fontSize: 13 }}>
          {dial.verbReading} · {dial.verbGloss}
        </span>. Turn the dial. Notice that <b style={{ color: "var(--accent)", fontStyle: "normal" }}>私</b> stays
        the subject the whole way — only its role keeps changing.
      </div>
          ...
          <div className="actor-stage">
            ...
            <div className="actor-arrow">
              ...
              <svg viewBox="0 0 64 30" aria-hidden="true">
                <defs>
                  <marker id={"ah-" + state.id} markerWidth="7" markerHeight="7" refX="5" refY="3"
                          orient="auto" markerUnits="strokeWidth">
                    <path d="M0,0 L6,3 L0,6 Z" fill="var(--accent)" />
                  </marker>
                </defs>
                {rightward ? (
                  <line x1="4" y1="15" x2="56" y2="15" stroke="var(--accent)" strokeWidth="1.6"
                        markerEnd={"url(#ah-" + state.id + ")"} />
                ) : (
                  <line x1="60" y1="15" x2="8" y2="15" stroke="var(--accent)" strokeWidth="1.6"
                        markerEnd={"url(#ah-" + state.id + ")"} />
                )}
              </svg>
            </div>
